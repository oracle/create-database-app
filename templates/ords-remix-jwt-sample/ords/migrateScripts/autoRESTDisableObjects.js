...
import { TABLE_OBJECT_TYPE } from './constants/autoREST.js';
/**
 ...
 */
async function autoRESTDisableObjects(schemaName, endpoint, basicAuth) {
  const restSQLEndpoint = `${endpoint}_/sql`;
  ...
  const autoRESTenableStatements = `
      BEGIN
          ${objectsToRESTEnable.filter((object) => object.isAutoRestAuth).map((object) => enableObject(schemaName, object.objectName, object.objectType, object.objectAlias, object.isAutoRestAuth, 'FALSE')).join(' ')}
          COMMIT;
      END;
      /
      `;
  const statementsResponse = await postRequest(
    restSQLEndpoint,
    autoRESTenableStatements,
    basicAuth,
  );
  printResponse(statementsResponse);
}

export default autoRESTDisableObjects;
