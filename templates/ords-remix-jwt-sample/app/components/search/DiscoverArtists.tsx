...
import { loader } from '~/routes/search.artists';
...
import ArtistsFilterSelector from './ArtistsFilterSelector';
import ArtistResultCard from './ArtistResultCard';
import SearchTooltip from '../tooltips/SearchTooltipComponent';

interface DiscoverArtistProps {
  id: string;
  title: string;
  description: string;
  enableTooltips: boolean;
  initialResultSet: ORDSResponse<ArtistResult>;
 ...
 */
function DiscoverArtists(props: DiscoverArtistProps) {
  const {
    id, title, description, enableTooltips, initialResultSet, musicGenres,
  } = props;
        ...
        </div>
        {
          fetcher.state === NavigationState.IDLE ? (
            <div className=" grid grid-cols-2 justify-evenly gap-4 py-4">
              { searchResult.items.map((result : ArtistResult, index: number) => (
                <div
                  key={result.name}
                >
                  <ArtistResultCard
